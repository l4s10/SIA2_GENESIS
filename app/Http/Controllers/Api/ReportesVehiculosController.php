...
class ReportesVehiculosController extends Controller
{
    ...
    public function getGraficos()
    {
        try{
            ...
            $grafico2 = $this->rankingVehiculosAsignados(new Request());
            $grafico3 = $this->rankingDepartamentosUbicaciones(new Request());
            $grafico4 = $this->rankingRevisoresSolicitudesVehiculos(new Request());
    ...
    public function filtrarGeneral(Request $request)
    {
        try{
            ...
            $grafico2 = $this->rankingVehiculosAsignados($request);
            $grafico3 = $this->rankingDepartamentosUbicaciones($request);
    ...
    /**
     * Método para obtener el ranking de solicitudes de vehículos.
     *
    ...
    public function rankingSolicitudes(Request $request)
    {
        try {
            ...
            if ($fechaInicio && $fechaFin) {
                $query->whereBetween('SOLICITUD_VEHICULO_FECHA_HORA_INICIO_SOLICITADA', [$fechaInicio, $fechaFin]);
            }

            $rankingSolicitudes = $query->groupBy('SOLICITUD_VEHICULO_ESTADO')
                ->select('SOLICITUD_VEHICULO_ESTADO', DB::raw('COUNT(*) as cantidad'))
                ->orderBy('cantidad', 'desc')
                ->get();

            return [
                'rankingSolicitudes' => $rankingSolicitudes,
            ];
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
    ...
    public function rankingVehiculosAsignados(Request $request)
    {
        try {
            ...
            $rankingVehiculos = $query->join('vehiculos', 'solicitudes_vehiculos.VEHICULO_ID', '=', 'vehiculos.VEHICULO_ID')
                ...
                ->select('vehiculos.VEHICULO_PATENTE', DB::raw('COUNT(*) as cantidad'))
                ->orderBy('cantidad', 'desc')
                ->get();
    ...
    public function rankingDepartamentosUbicaciones(Request $request)
    {
        try {
            ...
            $rankingDepartamentos = $queryDepartamentos->groupBy('departamentos.DEPARTAMENTO_ID', 'departamentos.DEPARTAMENTO_NOMBRE')
                ->select('departamentos.DEPARTAMENTO_NOMBRE as nombre', DB::raw('COUNT(*) as cantidad'))
                ->orderBy('cantidad', 'desc')
                ->get();

            $rankingUbicaciones = $queryUbicaciones->groupBy('ubicaciones.UBICACION_ID', 'ubicaciones.UBICACION_NOMBRE')
                ->select('ubicaciones.UBICACION_NOMBRE as nombre', DB::raw('COUNT(*) as cantidad'))
                ->orderBy('cantidad', 'desc')
                ->get();

            return [
                'rankingDepartamentos' => $rankingDepartamentos,
                'rankingUbicaciones' => $rankingUbicaciones,
            ];
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
    ...
    public function rankingRevisoresSolicitudesVehiculos(Request $request)
    {
        try {
            ...
            return [
                'rankingRevisores' => $rankingRevisores,
            ];
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
